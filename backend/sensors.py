...
I2C_SLAVE = 0x0703
STATE_MIN_WRITE_S = 0.25
STATE_KEEPALIVE_S = 2.0
_LAST_SENSOR_STATE_SIG: str | None = None
_LAST_SENSOR_STATE_WRITE = 0.0
...
def main() -> None:
    ...
    while True:
        ...
        now = time.time()
        payload = {"navigation": state, "power": power_state, "compass_calibration": cal_state, "updated_at": now}
        if now - last_write >= STATE_MIN_WRITE_S and _should_write_sensor_state(payload, now):
...
class Qmc5883l:
    def __init__(self, device: str, address: int) -> None:
        ...
        self._last_valid_raw: tuple[float, float, float] | None = None
    ...
    def _read_measurement(self) -> tuple[float, float, float] | None:
        ...
        for _ in range(3):
            try:
                ...
                if status is not None:
                    if status & 0x02:  # overflow
                        return None
                    if not (status & 0x01):  # data not ready yet
                        time.sleep(0.005)
                        continue
            ...
            if len(data) != 6:
                return None
            raw = tuple(float(v) for v in struct.unpack("<hhh", data))
    ...
    def _looks_valid(self, raw: tuple[float, float, float]) -> bool:
        if raw == (0.0, 0.0, 0.0):
            return False
        # Broken I2C frames from this setup often come back as one real axis and
        # two literal +/-1 values. Do not let those frames jump the heading.
        if sum(1 for v in raw if abs(v) <= 1.0) >= 2:
            return False
        if self._last_valid_raw is not None:
            max_delta = max(abs(raw[i] - self._last_valid_raw[i]) for i in range(3))
            if max_delta > 3500.0 and any(abs(v) <= 1.0 for v in raw):
                return False
        return True
...
class CompassCalibrator:
    ...
    def poll(self) -> dict[str, Any]:
        ...
        raw_list = [round(float(v), 1) for v in raw] if raw else None
        if raw:
            self.samples += 1
        ...
        stats = self._build_stats(raw_list)
        ranges = stats["ranges"]
        coverage = min(1.0, min(ranges) / self.MIN_AXIS_RANGE) if self.samples > 10 else 0.0
        sample_progress = min(1.0, self.samples / self.MIN_SAMPLES)
        progress = int(min(99.0, max(coverage, sample_progress * 0.75) * 100.0))

        if self.samples >= self.MIN_SAMPLES and coverage >= 0.65:
            self.active = False
            ...
            warnings = self._warnings(ranges, radii)
            self.last_result = {
                "offset": offset,
                "scale": scale,
                ...
                "max": stats["max"],
                "ranges": ranges,
                "radii": [round(v, 1) for v in radii],
                "quality": self._quality(ranges, radii),
                "warnings": warnings,
            }
    ...
    def _build_stats(self, raw: list[float] | None) -> dict[str, Any]:
        finite = all(math.isfinite(self.min_v[i]) and math.isfinite(self.max_v[i]) for i in range(3))
        if not finite:
            ranges = [0.0, 0.0, 0.0]
            radii = [0.0, 0.0, 0.0]
            return {
                ...
                "ranges": ranges,
                "quality": "idle",
                "warnings": ["Немає сирих даних компаса"],
            }
        ranges = [round(max(0.0, self.max_v[i] - self.min_v[i]), 1) for i in range(3)]
        radii = [max(1.0, r / 2.0) for r in ranges]
        return {
            "raw": raw,
            "min": [round(v, 1) for v in self.min_v],
            "max": [round(v, 1) for v in self.max_v],
            "ranges": ranges,
            "quality": self._quality(ranges, radii),
            "warnings": self._warnings(ranges, radii),
        }

    def _quality(self, ranges: list[float], radii: list[float]) -> str:
        if self.samples < 20:
            return "збираємо дані"
        min_range = min(ranges)
        spread = (max(radii) - min(radii)) / max(radii) if max(radii) > 0 else 1.0
        if min_range < self.MIN_AXIS_RANGE * 0.45:
            return "мало обертання по одній осі"
        if spread > 0.55:
            return "погано: метал або неправильні осі"
        if min_range >= self.MIN_AXIS_RANGE and spread < 0.28:
            return "добре"
        return "середньо"

    def _warnings(self, ranges: list[float], radii: list[float]) -> list[str]:
        warnings: list[str] = []
        if self.samples >= 20 and min(ranges) < self.MIN_AXIS_RANGE * 0.45:
            warnings.append("Поверни ровер по всіх осях: ніс вліво/вправо, нахил вперед/назад, крен.")
        if max(radii) > 0:
            spread = (max(radii) - min(radii)) / max(radii)
            if self.samples >= 60 and spread > 0.55:
                warnings.append("Підозра на метал/силові дроти поруч або помилкове X/Y/Z.")
        return warnings
...
def _write_state(state: dict[str, Any]) -> None:
    global _LAST_SENSOR_STATE_SIG, _LAST_SENSOR_STATE_WRITE
    ...
    _LAST_SENSOR_STATE_SIG = _sensor_state_signature(state)
    _LAST_SENSOR_STATE_WRITE = time.time()


def _should_write_sensor_state(state: dict[str, Any], now: float) -> bool:
    sig = _sensor_state_signature(state)
    if sig != _LAST_SENSOR_STATE_SIG:
        return True
    return now - _LAST_SENSOR_STATE_WRITE >= STATE_KEEPALIVE_S


def _sensor_state_signature(state: dict[str, Any]) -> str:
    return json.dumps(_compact_for_signature(state), sort_keys=True, separators=(",", ":"))


def _compact_for_signature(value: Any) -> Any:
    if isinstance(value, dict):
        return {k: _compact_for_signature(v) for k, v in value.items() if k not in {"updated_at", "updated_ts"}}
    if isinstance(value, list):
        return [_compact_for_signature(v) for v in value]
