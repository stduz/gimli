...
from backend.settings import CONTROL_STATE_FILE, SENSOR_COMMAND_FILE, SENSOR_STATE_FILE, load_settings, telemetry
...
def _handle_message(link: MavUdp, msg_id: int, payload: bytes, last_command: float, armed: bool) -> tuple[float, bool, bool]:
    if msg_id == 69 and len(payload) >= 11:
        x, y, _z, _r, _buttons, _target = struct.unpack_from("<hhhhHB", payload)
        armed = _handle_manual_buttons(link, _buttons, armed)
        if not armed:
            _post_control({"cmd": "stop"})
            return last_command, False, armed
        _post_control({"cmd": "drive", "throttle": _scale_axis(y), "steering": _scale_axis(x)})
...
def _handle_manual_buttons(link: MavUdp, buttons: int, armed: bool) -> bool:
    ...
    arm_on = _button(buttons, BTN_ARM)
    if BUTTON_STATE.get("armed_switch") is None or BUTTON_STATE.get("armed_switch") != arm_on:
        BUTTON_STATE["armed_switch"] = arm_on
        print(f"armed={bool(arm_on)} by button {BTN_ARM + 1}", flush=True)
        _send_status_text(link, f"ARM {'ON' if arm_on else 'OFF'}")
...
def _send_video_stream_information(link: MavUdp, stream_id: int = 0) -> None:
    ...
    streams = [("active", "active"), ("cam1", "front"), ("cam2", "rear")]
    if stream_id == 0:
...
def _write_sensor_command(command: str) -> None:
    try:
        ...
        SENSOR_COMMAND_FILE.write_text(json.dumps({"command": command, "time": time.time()}) + "\n", encoding="utf-8")
